...
function SellRequests() {
    ...
    const fetchAllData = async () => {
        try {
            ...
            const [requestsResponse, statsResponse] = await Promise.all([
                axiosInstance.get(`/api/v1/sell/admin/requests`),
                axiosInstance.get('/api/v1/sell/admin/requests/stats')
            ]);
        ...
        return () => {
            ...
            if (filters.search) {
                ...
                filtered = filtered.filter(request =>
                    ...
                    request.itemType.toLowerCase().includes(searchTerm) ||
            ...
            if (filters.preferredMethod !== "all") {
                filtered = filtered.filter(request => request.preferredMethod === filters.preferredMethod);
            }
    ...
    const updateRequestStatus = async (requestId, newStatus) => {
        try {
            const { data } = await axiosInstance.put(`/api/v1/sell/admin/requests/${requestId}`, {
                status: newStatus
            });
    ...
    const deleteRequest = async (requestId) => {
        if (!window.confirm("Are you sure you want to delete this selling request? This action cannot be undone.")) {
            return;
        }

        try {
            const { data } = await axiosInstance.delete(`/api/v1/sell/admin/requests/${requestId}`);
    ...
    const getStatusBadge = (status) => {
        const config = {
            ...
            "in-review": { color: "bg-purple-100 text-purple-800", icon: AlertCircle, text: "In Review" },
            contacted: { color: "bg-amber-100 text-amber-800", icon: CheckCircle, text: "Contacted" },
            approved: { color: "bg-green-100 text-green-800", icon: CheckCircle2, text: "Approved" },
            listed: { color: "bg-emerald-100 text-emerald-800", icon: Package, text: "Listed" },
    ...
    const getMethodBadge = (method) => {
        const config = {
            auction: { color: "bg-indigo-100 text-indigo-800", icon: Gavel, text: "Auction" },
            "buy-it-now": { color: "bg-green-100 text-green-800", icon: ShoppingCart, text: "Buy It Now" },
            "not-sure": { color: "bg-gray-100 text-gray-800", icon: HelpCircle, text: "Not Sure" }
        };
        const { color, icon: Icon, text } = config[method] || config["not-sure"];
        return (
            <span className={`px-2 py-1 rounded-full text-xs font-medium flex items-center gap-1 ${color}`}>
                <Icon size={12} />
                {text}
    ...
    const requestStats = {
        ...
        listed: filteredRequests.filter(r => r.status === 'listed').length
    };
    ...
    const statCards = [
        ...
        {
            title: "In Review",
            value: requestStats.inReview.toLocaleString('en-US'),
        ...
        {
            title: "Contacted",
            value: requestStats.contacted.toLocaleString('en-US'),
        ...
        {
            title: "Listed",
            value: requestStats.listed.toLocaleString('en-US'),
            change: "Successfully Listed",
            icon: CheckCircle2,
            color: "green"
        }
    ];
                ...
                <div className="w-full relative">
                    ...
                    <StaffContainer>
                        <div className="max-w-full pt-16 pb-7 md:pt-0">
                            <h2 className="text-3xl md:text-4xl font-bold my-5">Sell Requests</h2>
                ...
                <StaffContainer>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-6 gap-4">
                            ...
                            <div className="lg:col-span-4">
                                <div className="relative">
                                    ...
                                    <input
                                        type="text"
                                        placeholder="Search by name, email, or item type..."
                            ...
                            <div>
                                <select
                                    className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent"
                                    value={filters.preferredMethod}
                                    onChange={(e) => handleFilterChange('preferredMethod', e.target.value)}
                                >
                    ...
                    <div className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden mb-16">
                        <div className="px-6 py-4 border-b border-gray-200 bg-gray-50">
                            <h3 className="text-lg font-semibold">Sell Requests ({filteredRequests.length})</h3>
                        </div>
                        <div className="overflow-x-auto">
                                ...
                                <tbody className="divide-y divide-gray-200">
                                    {filteredRequests.map((request) => (
                                        <tr key={request.id} className="hover:bg-gray-50">
                                            ...
                                            <td className="py-4 px-6">
                                                {getMethodBadge(request.preferredMethod)}
                                            </td>
                                            <td className="py-4 px-6">
                            ...
                            {filteredRequests.length === 0 && (
                                <div className="text-center py-12">
                                    ...
                                    <p className="text-gray-500">No selling requests found matching your criteria</p>
                                    <button
                                        onClick={clearFilters}
                        ...
                        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
                            <div className="bg-white rounded-xl shadow-lg max-w-3xl w-full max-h-[90vh] overflow-y-auto">
                                <div className="px-6 py-4 border-b border-gray-200 flex justify-between items-center sticky top-0 bg-white z-10">
                                    <h3 className="text-lg font-semibold">Selling Request - {selectedRequest.requestId}</h3>
                                    <button
                                ...
                                <div className="p-6">
                                    ...
                                    <div className="flex items-start gap-4 mb-6">
                                        ...
                                        <div className="flex-1">
                                            ...
                                            <div className="flex flex-wrap gap-2 mb-2">
                                                ...
                                                {getMethodBadge(selectedRequest.preferredMethod)}
                                            </div>
                                    ...
                                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                                        ...
                                        <div className="space-y-4">
                                            <h5 className="font-semibold text-gray-900">Item Details</h5>
                                            <div className="space-y-3">
                                                <div className="flex justify-between">
                                                    <span className="text-gray-500">Item Type</span>
                                                    <span className="font-medium">{selectedRequest.itemType}</span>
                                                </div>
                                                <div className="flex justify-between">
                                                    <span className="text-gray-500">Preferred Method</span>
                                                    <span className="font-medium capitalize">{selectedRequest.preferredMethod?.replace('-', ' ')}</span>
                                                </div>
                                    ...
                                    <div className="flex gap-3 pt-6 border-t border-gray-200">
                                        <a
                                            href={`mailto:${selectedRequest.email}?subject=Re: Selling Request (${selectedRequest.requestId})&body=Dear ${selectedRequest.name},%0D%0A%0D%0AThank you for your selling request regarding: ${selectedRequest.itemType}%0D%0A%0D%0A`}
                                    ...
                                    <div className="flex flex-wrap gap-2 mt-4">
                                        {selectedRequest.status === "new" && (
                                            <>
                                                <button
                                                    onClick={() => {
                                                        updateRequestStatus(selectedRequest.id, "in-review");
                                                        closeRequestModal();
                                                    }}
                                        ...
                                        {/* {selectedRequest.status === "contacted" && (
                                            <>
                                                ...
                                                <button
                                                    onClick={() => {
                                                        updateRequestStatus(selectedRequest.id, "listed");
                                                        closeRequestModal();
                                                    }}
